...
def loading_branch(self):
    # 生成递增branch_no
    lineEdit = self.form5.findChild(QLineEdit, "lineEdit")
    createBranchNoSql = "SELECT branch_no FROM t_iccard_branch where deleted=0 ORDER BY branch_no DESC LIMIT 1"
    ...
    for data in result:
        branch_no = data[0]
        lineEdit.setText(str(int(branch_no)+1))
