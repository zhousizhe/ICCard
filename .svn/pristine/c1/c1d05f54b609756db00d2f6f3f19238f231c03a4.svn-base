...
def loading(self):
    ...
    manageInfomationSql = "SELECT branch_no, name, telephone, person, province, city, district, description FROM t_iccard_branch WHERE deleted = 0"
...
def modify_branch(self):
    ...
    try:
        # 查找省索引供下拉框设置当前索引
        indexprovinces = provincesBox.findText(dateList[4])
    ...
    try:
        # 查找市索引供下拉框设置当前索引
        indexcitys = citysBox.findText(dateList[5])
    except BaseException: QMessageBox.warning(self, "错误", "系统错误", QMessageBox.Close)
    ...
    try:
        # 查找区索引供下拉框设置当前索引
        indexdistricts = districtsBox.findText(dateList[6])
    except BaseException: QMessageBox.warning(self, "错误", "系统错误", QMessageBox.Close)
    districtsBox.setCurrentIndex(indexdistricts)

    self.updateBanch.findChild(QLineEdit, "branchNum").setText(dateList[0])
    self.updateBanch.findChild(QLineEdit, "names").setText(dateList[1])
    self.updateBanch.findChild(QLineEdit, "phone").setText(dateList[2])
    self.updateBanch.findChild(QLineEdit, "person").setText(dateList[3])


    self.updateBanch.findChild(QLineEdit, "lineEdit_9").setText(dateList[7])
...
def revoke_branch(self):
    ...
    for range in selectedRanges:
        ...
        indexBranchNo = modle.index(row, 0)  # 列数据:BranchNo
        ...
        dataBranchNoList.append(dataBranchNo)
        tempsize = tempsize+1
    ...
    if reply == QMessageBox.Yes:
        for branchNo in dataBranchNoList:
            updateBranchSql = "UPDATE t_iccard_branch SET deleted = %d  where branch_no='%s'" % (1, branchNo)
            try:
...
def serach_branch(self):
    ...
    branchInformationSql = "SELECT branch_no, name, telephone, person, province, city, district, description FROM t_iccard_branch WHERE 1=1 and deleted = 0"
    paralist = []
