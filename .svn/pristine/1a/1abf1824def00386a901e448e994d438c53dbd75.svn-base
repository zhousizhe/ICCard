...
def loading_assistant(self):
    ...
    assistantInfomationSql = "SELECT operator_id, user_name, real_name, id_number, phone, branch_no, branch_name FROM t_user_base WHERE deleted = 0"
...
def serach_assistant(self):
    ...
    assistantInformationSql = "SELECT  operator_id, user_name, real_name, id_number, phone, branch_no, branch_name " \
...
def remove_assistant(self):
    ...
    if reply == QMessageBox.Yes:
        for id in idList:
            updateBranchSql = "UPDATE t_user_base SET deleted = %d  where operator_id='%s'" % (1, id)
            try:
...
def update_assistant(self):
    ...
    self.updateAssistant.findChild(QLineEdit, "assistantNum").setText(dateList[0])
    self.updateAssistant.findChild(QLineEdit, "userName").setText(dateList[1])
    self.updateAssistant.findChild(QLineEdit, "realName").setText(dateList[2])
    # self.updateAssistant.findChild(QComboBox, "rolrType")
    self.updateAssistant.findChild(QLineEdit, "phone").setText(dateList[4])
    self.updateAssistant.findChild(QLineEdit, "branchNum").setText(dateList[5])
    self.updateAssistant.findChild(QLineEdit, "branchName").setText(dateList[6])